...
// Helper for Role colors (GK, FW, etc)
  ...
  return (
    <div className="rounded-md border border-slate-800 bg-slate-950 shadow-sm overflow-hidden">
      <Table>
        ...
        <TableBody>
          {players.map((player) => (
            <TableRow key={player.id} className="border-slate-800 hover:bg-slate-900/50 group">
              <TableCell>
              ...
              <TableCell className={`font-black italic ${getRoleColor(player.role)}`}>
                {player.role}
              </TableCell>
